...
function Model_Attach_addAttch($page_srl, $category, $doc_srl, $user_srl, $kind, $filename, $extension , $filevalue, $size, $status){
   return DBQuery("INSERT INTO `attach` (`page_srl`, `doc_srl`, `user_srl`, `kind`, `filename`, `extension`, `filevalue`, `date`, `size`, `status` , `ip_addr`) VALUES ('$page_srl', '$doc_srl', '$user_srl', '$kind', '$filename', '$extension', '$filevalue', '" . getTimeStamp() . "', '$size', '$status','" . getIPAddr() . "');");
}
