...
//        security_value_check($content);
        ...
        for ($i = 0; $i < count($square_cards_array); $i++) {
            ...
            $SQUARE_CARD_CLASS->Write($ATTACH_CLASS, $PUSH_CLASS, $last_number, 0, $page_srl, $user_srl, RealEscapeString($square_cards_array[$i]['content']), RealEscapeString($square_cards_array[$i]['content']), RealEscapeString($square_cards_array[$i]['align']), RealEscapeString($square_cards_array[$i]['style']), $permission, $status, $privacy);
...
//echo mysqli_error();
    ...
    function getMain($SQUARE_CLASS, $PAGE_CLASS, $ATTACH_CLASS, $user_srl, $square_info, $attach_info)
    {
        ...
        $top_items = array(array("square_key" => "birthday", "title" => "생일 축하 카드 만들기"), array("square_key" => "http://tarks.net/fsquare", "title" => "Square Forum 방문하기"), array("square_key" => "square38patch", "title" => "Square 3.8 패치 노트"));
