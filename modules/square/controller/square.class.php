...
//Status
    ...
    {
        $cards = getSqlList($SQUARE_CARD_CLASS->Read($square_srl), array("content", "date", "style", "align", "background"));
...
//        security_value_check($content);
        ...
        for ($i = 0; $i < count($square_cards_array); $i++) {
            ...
            $SQUARE_CARD_CLASS->Write($ATTACH_CLASS, $PUSH_CLASS, $last_number, 0, $page_srl, $user_srl, RealEscapeString($square_cards_array[$i]['content']), RealEscapeString($square_cards_array[$i]['align']), RealEscapeString($square_cards_array[$i]['style']), RealEscapeString($square_cards_array[$i]['background']), $permission, $status, $privacy, 0);
...
//echo mysqli_error();
    ...
    function getMain($SQUARE_CLASS, $PAGE_CLASS, $ATTACH_CLASS, $user_srl, $square_info, $attach_info)
    {
        ...
        $top_items = array(array("square_key" => "birthday", "title" => "생일 축하 카드 만들기"), array("square_key" => "s9uare0316", "title" => "3월 16일 변경 사항"));
