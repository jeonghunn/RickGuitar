...
function Model_Square_Write($square_key, $page_srl, $user_srl, $channel, $name, $title, $content, $type, $data, $keep_square, $permission, $status, $privacy, $attach_result)
{
    return DBQuery("INSERT INTO `square` (`square_key`, `page_srl`, `user_srl`,  `channel`,  `name`, `title`, `content`,`type` ,`data`, `keep_square`, `date`, `permission`, `status`, `privacy`,  `attach`,  `ip_addr`) VALUES ('$square_key', '$page_srl', '$user_srl' ,'$channel', '$name', '$title', '$content', '$type', '$data', '$keep_square',  '" . getTimeStamp() . "', '$permission', '$status', '$privacy', '$attach_result ? 1 : 0', '" . getIPAddr() . "');");
}
...
{
    return DBQuery("SELECT * FROM  `square` WHERE `channel` LIKE '$channel' AND (`status` LIKE '0' OR (`user_srl` =$user_srl AND `status` < 5 AND `user_srl` != 0)) ORDER BY  `square`.`srl` DESC LIMIT $start , $number");
}
